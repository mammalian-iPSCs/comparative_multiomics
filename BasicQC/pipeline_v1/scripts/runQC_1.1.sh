...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 </path/to/fastqs/> <list_of_fastq_files.xls> <project (e.g. CGLZOO_01)> <type (e.g. RNA-seq)>"
  exit 1
fi
...
PROJECT="$3"
TYPE="$4"
FASTQ_LIST="${PROJECT}.fastqs.txt"

# Create the main project directory in the QC directory
cd /scratch_isilon/groups/compgen/data/Illumina_CryoZoo/BasicQC
mkdir -p "$PROJECT"
WORKDIR=/scratch_isilon/groups/compgen/data/Illumina_CryoZoo/BasicQC/$PROJECT

# Define subfolder names
SUBFOLDERS=("fastqc" "fastq_screen" "Rplots" "logs" "slurm")
...
echo "$(date) starting job Plot_QC.R"
# Rplots - SLURM job submission for QC Plot
sbatch -J Plot_Cov_${PROJECT} --output=Plot_Cov_%j.out --error=Plot_Cov_%j.err --mem 5G --partition=genD --wrap "Rscript scripts/Plot_QC.R ../../${PROJECT}_sample_info.txt $PROJECT $PROJPATH $TYPE"
